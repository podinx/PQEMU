    ...
    switch (delivery_mode) {
        case APIC_DM_LOWPRI:
            /* XXX: search for focus processor, arbitration */
            {
                ...
                if (d >= 0) {
                    ...
                    if (apic_iter) {
                        push_interrupt(apic_iter, vector_num, trigger_mode, Q_APIC_SET_IRQ, apic_iter->id);
        ...
        case APIC_DM_SMI:
            foreach_apic(apic_iter, deliver_bitmask,
                push_interrupt(apic_iter->cpu_env, CPU_INTERRUPT_SMI, -1, Q_CPU_INTERRUPT, apic_iter->id));
                //cpu_interrupt(apic_iter->cpu_env, CPU_INTERRUPT_SMI) );
            return;

        case APIC_DM_NMI:
            foreach_apic(apic_iter, deliver_bitmask,
                push_interrupt(apic_iter->cpu_env, CPU_INTERRUPT_NMI, -1, Q_CPU_INTERRUPT, apic_iter->id));
        ...
        case APIC_DM_INIT:
            ...
            foreach_apic(apic_iter, deliver_bitmask,
                push_interrupt(apic_iter->cpu_env, CPU_INTERRUPT_INIT, -1, Q_CPU_INTERRUPT, apic_iter->id));
                         ...
                         uint8_t polarity, uint8_t trigger_mode)
{
    ...
    switch (delivery_mode) {
        case APIC_DM_INIT:
            {
                ...
                if (level == 0 && trig_mode == 1) {
                    foreach_apic(apic_iter, deliver_bitmask,
                                 push_interrupt(apic_iter, -1, -1, Q_ARB_ID, apic_iter->id));
        ...
        case APIC_DM_SIPI:
            foreach_apic(apic_iter, deliver_bitmask,
                         push_interrupt(apic_iter, vector_num, -1, Q_APIC_STARTUP, apic_iter->id));
...
static void apic_timer(void *opaque)
{
    ...
    qemu_mutex_lock(&env->pvc_timer_mutex);
    apic_local_deliver(s, APIC_LVT_TIMER);
    qemu_mutex_unlock(&env->pvc_timer_mutex);
...
void run_interrupt(void *s, int vector_num, int trigger_mode, int fun_type)
{
    if(fun_type == Q_APIC_SET_IRQ)
    {
        APICState *apic_iter = s;
        apic_set_irq(apic_iter, vector_num, trigger_mode);
    }
    else if(fun_type == Q_CPU_INTERRUPT)
    {
        CPUState *env = s;
        cpu_interrupt(env, vector_num);
    }
    else if(fun_type == Q_APIC_STARTUP)
    {
        APICState *apic_iter = s;
        apic_startup(apic_iter, vector_num);
    }
    else if(fun_type == Q_ARB_ID)
    {
        APICState *apic_iter = s;
        apic_iter->arb_id = apic_iter->id;
    }
...
void *apic2env(void *s)
{
    APICState *apic = s;
    CPUState *env = apic->cpu_env;
